...
@Suite("HTTPClient Configuration") struct HTTPClientConfigurationTests {
    ...
    private func makeEngine(configuration: DefaultHTTPClient.Configuration = .default) -> (DefaultHTTPClient, MockURLProtocol.MockContext) {
        let (session, mock) = MockURLProtocol.makePair()
        return (DefaultHTTPClient(session: session, configuration: configuration), mock)
    }
    ...
    @Test func concurrentRequestsCarryOwnConfiguration() async throws {
        ...
        let engineB = DefaultHTTPClient(session: sessionB)
