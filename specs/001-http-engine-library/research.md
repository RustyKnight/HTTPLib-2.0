...
## Decision 2 — `HTTPEngine` as a struct (value type)

**Decision**: Implement `HTTPEngine` as a `public struct` with `let` stored
properties.
...
concurrent use of the same `HTTPEngine` instance is safe — no per-instance mutable
...
struct is strictly simpler. Protocol `HTTPEngineProtocol` — deferred per A-01; a
...
**Rationale**: `HTTPEngine` (a struct) synthesises `Sendable`. Swift 6 requires all
...
`CancellationError` in `HTTPEngineError`; let it propagate directly to the caller.
...
unchanged; wrapping it in `HTTPEngineError` would break callers who specifically
...
| `HTTPEngine` value vs reference type | `struct` — synthesises `Sendable`, no shared mutable state |
